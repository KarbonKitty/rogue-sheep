...
{
    /// <summary>
    /// Algorithm developed by Adam Milazzo
    /// Found here: http://www.adammil.net/blog/v125_Roguelike_Vision_Algorithms.html#mycode
    /// </summary>
    public class BevelledWallShadowcasting
    {
        private readonly GameGrid<bool> _transparencyGrid;

        public BevelledWallShadowcasting(GameGrid<bool> transparencyGrid)
        {
            _transparencyGrid = transparencyGrid;
        }

        public GameGrid<bool> Compute(Point2i origin, int rangeLimit)
        {
            var visibilityGrid = new GameGrid<bool>(_transparencyGrid.Size);
            visibilityGrid[origin] = true;
            for (uint octant = 0; octant < 8; octant++)
            {
                Compute(visibilityGrid, octant, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
            }
        ...
        private struct Slope // represents the slope Y/X as a rational number
        {
            ...
            public bool Greater(uint y, uint x) => Y * x > X * y; // this > y/x
            public bool GreaterOrEqual(uint y, uint x) => Y * x >= X * y;  // this >= y/x
            public bool Less(uint y, uint x) => Y * x < X * y; // this < y/x
        }

        private void Compute(GameGrid<bool> visibilityGrid, uint octant, Point2i origin, int rangeLimit, uint x, Slope top, Slope bottom)
        {
            ...
            for (; x <= (uint)rangeLimit; x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
            {
                ...
                for (uint y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
                {
                    if (rangeLimit < 0 || GetDistance((int)x, (int)y) <= rangeLimit) // skip the tile if it's out of visual range
                    {
                        ...
                        if (isVisible)
                        {
                            SetVisible(visibilityGrid, x, y, octant, origin);
                        }
                            ...
                            if (isOpaque)
                            {
                                ...
                                {                  // so adjust the bottom vector upward and continue processing it in the next column
                                    ...
                                    if (top.Greater(ny, nx))
                                    {
                                        ...
                                        {
                                            Compute(visibilityGrid, octant, origin, rangeLimit, x + 1, top, new Slope(ny, nx));
                                        }
                                    }
        ...
        private bool BlocksLight(uint x, uint y, uint octant, Point2i origin)
        {
            ...
            return _transparencyGrid.IsInBounds(((int)nx, (int)ny)) && !_transparencyGrid[((int)nx, (int)ny)];
        }

        private void SetVisible(GameGrid<bool> visibilityGrid, uint x, uint y, uint octant, Point2i origin)
        {
            uint nx = (uint)origin.X, ny = (uint)origin.Y;
            switch (octant)
            {
